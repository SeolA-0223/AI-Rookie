...
const KO_COPY = {
  ...
  source: {
    ...
    liveSummary: "실제 조례명을 검색하고 최신 조례 목록을 보며 before/after ID를 채워 비교 분석할 수 있습니다.",
    sampleTitle: "샘플 케이스 비교",
    sampleBody: "번들 케이스를 선택하면 별도 ID 없이 바로 조례 변경 분석을 실행합니다.",
    liveTitle: "실제 조례 조회와 pair 비교",
    liveBody: "조례명을 검색해 추천 pair를 적용하거나, 최신 조례 목록에서 현재 버전을 골라 비교 분석할 수 있습니다.",
    providerLabel: "조회 소스",
    caseLabel: "샘플 케이스",
    searchLabel: "조례명 검색",
    ...
    searchHint: "조례명을 검색하면 후보 ID와 추천 before/after 조합을 보여줍니다.",
    municipalityTitle: "지자체 선택",
    latestHint: "선택한 지자체 기준으로 최신 공포 조례를 날짜순으로 불러옵니다.",
  ...
  messages: {
    ...
    sampleHelp: (title, municipality) =>
      title
        ...
        : "샘플 케이스를 선택하세요.",
    liveLawHelp: "law.go.kr 공개 소스로 검색합니다. 검색 결과가 부족하면 ID를 직접 입력하세요.",
    liveMcpHelp: "Korea Law MCP가 연결되어 있으면 검색과 추천 pair 적용이 가능합니다.",
    sourceSearchReady: "조례명을 검색하거나 known before/after ID를 입력하세요.",
    sourceSearchUnavailable: "샘플 모드에서는 검색이 필요하지 않습니다.",
    sourceSearchNeedQuery: "검색어를 먼저 입력하세요.",
    sourceSearchLoading: "조례 후보 검색 중...",
    sourceSearchLoaded: (count, hasRecommendation) =>
      hasRecommendation ? `후보 ${count}건과 추천 조합을 불러왔습니다.` : `후보 ${count}건을 불러왔습니다.`,
    ...
    latestAfterSelected: (id) => `${id}를 개정 버전 ID로 넣었습니다.`,
    ...
    selectedBefore: (id) => `${id}를 이전 버전으로 선택했습니다.`,
    selectedAfter: (id) => `${id}를 개정 버전으로 선택했습니다.`,
...
const EN_COPY = {
  ...
  source: {
    ...
    liveSummary: "Search real ordinance titles, browse the latest list, and fill before/after IDs for pair comparison.",
    ...
    liveBody: "Search by ordinance title, apply a recommended pair, or use the latest list to choose the current version.",
    ...
    searchHint: "Search an ordinance title to see candidate IDs and a recommended pair.",
    municipalityTitle: "Municipality filters",
    latestHint: "Load the most recently promulgated ordinances for the checked municipalities.",
  ...
  messages: {
    ...
    liveLawHelp: "Uses public law.go.kr search. Enter IDs directly if the search is incomplete.",
    liveMcpHelp: "If the Korea Law MCP endpoint is available, search and recommended-pair flows can use it directly.",
    ...
    latestAfterSelected: (id) => `Placed ${id} into the current-version field.`,
